...
const std::string CSyncCheckpoint::strMainPubKey = "0488F933334F99FFB4E3D9285A687C1D35E559F7A33950DCCBEBB344B442CE09E840B08F112C4B699CFE4D78341040948F976D521E18B327526FE1987E5509249D";
const std::string CSyncCheckpoint::strTestPubKey = "04D7AA290722E9A350D58F68D78747BA1158CEE72248B2A2FE041AFA4FD916F73E610F1C3B281A8569A820ECE692E0540FDD592201964DA4368B0BABB80C767528";
...
//            if (!ConnectTip(state, pindexCheckpoint))ActivateBestChain
...
uint256 AutoSelectSyncCheckpoint()
{
    ...
    while (pindex->pprev && pindex->nHeight + (int)GetArg("-checkpointdepth", 0) > chainActive.Tip()->nHeight)
        pindex = pindex->pprev;
...
//        if (!SetBestChain(state, mapBlockIndex[hash]))
    ...
    if (!WriteSyncCheckpoint((mapBlockIndex.count(hash) && chainActive.Contains(mapBlockIndex[hash]))? hash : Params().HashGenesisBlock()))
...
bool SetCheckpointPrivKey(std::string strPrivKey)
{
	...
	bool psj=vchSecret.SetString(strPrivKey);
	LogPrintf("privkey:%d\n", strPrivKey);
	LogPrintf("privkeyvch:%s\n", psj);
...
bool SendSyncCheckpoint(uint256 hashCheckpoint)
{
    ...
    CKey key = vchSecret.GetKey(); // if key is not correct openssl may crash
    if (!key.Sign(Hash(checkpoint.vchMsg.begin(), checkpoint.vchMsg.end()), checkpoint.vchSig))
        return error("SendSyncCheckpoint: Unable to sign checkpoint, check private key? Code 3");
